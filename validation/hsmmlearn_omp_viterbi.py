...
import numpy as np
import time
from hsmmlearn_omp.emissions import AbstractEmissions
from hsmmlearn_omp.hsmm import HSMMModel
...
def benchmark_baseline(json_file: str, csv_path="benchmark.csv", iterations=100, n_states=0, timesteps=0, max_duration=0):
    ...
    with open(csv_path, "a", newline="") as f:
        ...
        for i, t in enumerate(times):
            writer.writerow(["HSMMLearn_OMP", n_states, timesteps, max_duration, i, f"{t:.6f}"])

    avg, mn, mx = sum(times) / len(times), min(times), max(times)
    print(f"  {WHITE}HSMMLearn OMP{R}")
