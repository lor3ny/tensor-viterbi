                 ...
                 double dPara[], double pPara[], double piPara[], double pdfPara[], int hiddenStatesPara[])
{
    ...
    for (t = 0; t <= tau - 1; t++) {
        // Each j writes only to alpha[j][t], maxU[j][t], maxI[j][t] and reads
        // only from alpha[i][t-u] (u >= 1, already finalized). Safe to parallelize.
        #pragma omp parallel for private(u, i, k, x, Observ, first_alpha, first_i)
        for (j = 0; j <= J - 1; j++) {
            Observ = 0;
            first_alpha = true;
            for (u = 1; u <= min(t, M); u++) {
    ...
    // Same independence argument as above: each j writes to its own alpha[j][tau-1].
    #pragma omp parallel for private(u, i, k, x, Observ, first_alpha, first_i)
    for (j = 0; j <= J - 1; j++)
    {
        Observ = 0;
